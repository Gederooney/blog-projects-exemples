class Post {
  ...
  getMediasHtml(type = "image") {
    const areas = ["a", "b", "c", "d", "e", "f"];
    let html = "";
    if (type === "image")
      for (const [i, media] of this.medias.slice(0, 6).entries()) {
        html += `<div class="area h-full" style="--area-value: ${areas[i]}">
          <img class="object-cover object-center w-full h-full" src="${media.dataUri}" alt="">
        </div>`;
      }
    return html;
  }
  ...
  updatePreview() {
    ...
    if (this.medias.length) {
      ...
      const html = this.getMediasHtml(this.medias[0].type);
      this.previewImages.classList.remove(this.currentClass);
      this.previewImages.classList.add(style);
      this.currentClass = style;
